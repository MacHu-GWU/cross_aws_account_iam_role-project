...
iam_root_arn = x_aws_acc.IamRootArn(account=grantee_1_bsm.aws_account_id)
grantee_1 = x_aws_acc.Grantee(
    bsm=grantee_1_bsm,
    stack_name=f"{prefix}cross-aws-account-iam-role-iam-root-principal-grantee",
    iam_arn=iam_root_arn,
    policy_name=f"{prefix}cross_aws_account_iam_role_iam_root_principal_grantee",
)

owner_1_bsm = BotoSesManager(profile_name="bmt_app_test_us_east_1")
owner_1 = x_aws_acc.Owner(
    bsm=owner_1_bsm,
    stack_name=f"{prefix}cross-aws-account-iam-role-iam-root-principal-test-owner",
    role_name=f"{prefix}cross_aws_account_iam_role_iam_root_principal_test_owner",
    policy_name=f"{prefix}cross_aws_account_iam_role_iam_root_principal_test_owner",
    policy_document={
        "Version": "2012-10-17",
        "Statement": [
            {
